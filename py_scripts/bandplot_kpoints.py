...
def kpoints(NPts, route,):
    ...
    output = ''
    dists = [np.linalg.norm(kvecFromName(a)-kvecFromName(b))
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        ...
        print('\tL# are the labels of high symmetry points\n')
        exit()
    if sys.argv[1].isdigit():
        ...
        exit()
    if len(sys.argv) == 2:
        # route = ['Gamma', 'X', 'W', 'K', 'Gamma', 'L', 'K', 'W', 'L', 'U',
        #          'X', 'U', 'W']
        # route = ['Gamma', 'X', 'W', 'K', 'Gamma', 'L', 'U', 'W', 'L', 'K']
        # ['U', 'X']
