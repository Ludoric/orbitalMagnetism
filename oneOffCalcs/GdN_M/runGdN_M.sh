...
# mpirun -np 64 "$BINLOC/pw.x" -npool 4 -in GdN_M_nscf.pw.in > GdN_M_nscf.pw.out
# mpirun -np 64 "$BINLOC/pw.x" -npool 4 -in GdN_M_bands.pw.in > GdN_M_bands.pw.out
...
# # $BINLOC/bands.x < GdN_M_S1.bands.in >  GdN_M_S1.bands.out
# # $BINLOC/bands.x < GdN_M_S2.bands.in >  GdN_M_S2.bands.out
# # # Run wannier90 to generate a list of the required overlaps (written into the Fe.nnkp file).
# # # !!!! mpi not available for wannier90.x
# # $BINLOC/wannier90.x -pp GdN_M_up > GdN_M_dw_1.wannier90.out
# $BINLOC/wannier90.x -pp GdN_M_dw > GdN_M_dw_1.wannier90.out
...
# # # mpirun -np 25 "$BINLOC/pw2wannier90.x" -in GdN_M_up.pw2wan.in > GdN_M_up.pw2wan.out
# # # mpirun -np 25 "$BINLOC/pw2wannier90.x" -in GdN_M_dw.pw2wan.in > GdN_M_dw.pw2wan.out
# # $BINLOC/pw2wannier90.x < GdN_M_up.pw2wan.in > GdN_M_up.pw2wan.out
# $BINLOC/pw2wannier90.x < GdN_M_dw.pw2wan.in > GdN_M_dw.pw2wan.out
# # # Run wannier90 to compute the MLWFs.
# # # !!!! mpi not available for wannier90.x
# # $BINLOC/wannier90.x GdN_M_up > GdN_M_up_2.wannier90.out
# $BINLOC/wannier90.x GdN_M_dw > GdN_M_dw_2.wannier90.out
